 ...
 * and effects. The user can select which abilities the status affects, and in what
 * manner (add, override, downgrade, upgrade). The dialog includes a preview of the status item's image.
 *
 * If a target is selected, the created status item is created as an embedded document
 * on the token's actor and is also stored in the "customstatuses" compendium.
 * If no target is selected, the status item is only created in the compendium.
 *
 * Utilizes `game.erps.getTargetArray` to get the list of targeted tokens.
 * Uses a Handlebars template for rendering the dialog content.
 */
const statusCreator = async () => {
  ...
  const dialog = new Dialog({
    ...
    buttons: {
      yes: {
        label: `Create Status Item ${
          targetArray.length > 0 ? "and Apply" : ""
        }`,
        callback: async (html) => {
    ...
    render: async (html) => {
      ...
      const fileStatus = await FilePicker.fromButton(
        html.find("#file-picker")[0]
      );
