...
const rollHandler = async (dataSet, actor, cardData) => {
  ...
  if (targetArray.length) {
    label = `${label} <p class="eventide-card-effects-title">AC Check:</p>`;
    for (const target of targetArray) {
      label = `${label} <div class="eventide-card-effects flexrow">${target.actor.name}:`;
      ...
      label = `${label} </div>`;
      // targetRollData.abilities[ability].total+11
    }
  }
