...
const rollHandler = async (dataSet, actor) => {
  ...
  let card = dataSet.label
    ? `<div class="eventide-roll-card ${pickedCardClass}"><i class="${pickedIcon}"></i> ${dataSet.label}</div>`
    : "";
  ...
  if (critAllowed && dataSet.formula.includes("d")) {
    ...
    if (
      ...
      card = `${card} <p class="eventide-roll-card eventide-critical-hit"><i class="fa-solid fa-meteor"></i> Critical Hit!</p>`;
    } else if (
      dieRoll >= rollData.hiddenAbilities.fmin.total &&
      dieRoll <= rollData.hiddenAbilities.fmax.total
    ) {
      card = `${card} <p class="eventide-roll-card eventide-critical-miss"><i class="fa-solid fa-flag"></i> Critical Miss!</p>`;
    }
  }

  if (targetArray.length && acCheck) {
    card = `${card} <div class="ac-check-container"><p class="eventide-card-effects-title">AC Check:</p>`;
